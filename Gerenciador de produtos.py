...
def cadastrar_pedido():
    ...
    if cliente ==""or data == "":
        ...
        return
    cursor.execute("""
    insert into pedidos(cliente, data) values(?, ?)
    """, (cliente, data))
    conexao.commit()
...
def excluir_pedido():
    ...
    dados = tabela_pedidos.item(selecionado)
    id_pedido = dados["values"][0]
    resposta = messagebox.askyesno("Confirmação", "Tem certeza que deseja excluir este pedido?")
    if resposta:
        cursor.execute("""
...
def listar_itens():
    selecionado = tabela_pedidos.focus()
    if selecionado == "":
        messagebox.showerror("Atenção", "Selecione um pedido")
        return
    dados = tabela_pedidos.item(selecionado)
    id_pedido = dados["values"][0]
...
frame_pedido.pack(fill="x", padx=20, pady=10)
tk.Label(
    frame_pedido,
    text="Cliente:",
    bg="#f2f2f2"
).grid(row=0, column=0)
entrada_cliente = tk.Entry(frame_pedido, width=30)
...
entrada_data = tk.Entry(frame_pedido, width=20)
entrada_data.grid(
    row=0,
    column=3,
    padx=5
...
tabela_pedidos.bind("<ButtonRelease-1>", selecionar_pedido)
frame_itens = tk.LabelFrame( janela, text="Adicionar item ao pedido", font=("Arial", 14, "bold"), bg="#f2f2f2", padx=10, pady=10)
frame_itens.pack(fill="x", padx=20, pady=10)
tk.Label( frame_itens, text="id do pedido:", bg="#f2f2f2").grid(row=0, column=0)
...
entrada_produto = tk.Entry(frame_itens, width=30)
...
botao_listar = tk.Button( janela, text="Listar itens do pedido", bg="#34495e", fg="white", font=("Arial", 12), width=25,command=listar_itens)
